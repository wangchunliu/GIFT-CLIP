...
# ─────────────────────────────────────────────────────────────
# 动态计算项目根目录
# dataloader.py 位于 <PROJECT_ROOT>/model/dataloader.py
...
class Mydataset(Dataset):
    ...
    def __getitem__(self, item):
        ...
        img = Image.open(os.path.join(COCO_DATA_DIR, img_path)).convert('RGB')  # ← 修改
...
class VG_Attribution_Test(Dataset):
    def __init__(self, image_preprocess):
        ...
        with open(os.path.join(DATA_DIR, "visual_genome_attribution_aug.json"), "r") as f:  # ← 修改
            self.dataset = json.load(f)
