...
class myTransformer(nn.Module):
    ...
    def forward(self, head_word_list, relation_word_list, tail_word_list):
...
class triple_Transformer(nn.Module):
    ...
    def forward(self,head_inputs, relation_inputs, tail_inputs, token_type_id, attention_mask):
        text_emb = self.model(
            input_ids = head_inputs["input_ids"].reshape(-1, self.length).cuda(),  
            input_ids_rel = relation_inputs["input_ids"].reshape(-1, self.length).cuda(),
            input_ids_tail = tail_inputs["input_ids"].reshape(-1, self.length).cuda(),
            token_type_ids = head_inputs["token_type_ids"].reshape(-1, self.length).cuda(), 
            attention_mask = attention_mask)
...
class VisionTransformer(nn.Module):
    ...
    def forward(self, x: torch.Tensor):
        ...
        if self.proj is not None:
            proj = self.proj
            if hasattr(self, "proj_lora_A") and hasattr(self, "proj_lora_B"):
                proj = proj + (self.proj_lora_A @ self.proj_lora_B) * self.proj_lora_scale
            x = x @ proj
...
class CLIP(nn.Module):
    ...
    def encode_text(self, text):
        ...
        if hasattr(self, "text_projection_lora_A") and hasattr(self, "text_projection_lora_B"):
            text_projection = text_projection + (
                self.text_projection_lora_A @ self.text_projection_lora_B
            ) * self.text_projection_lora_scale
