...
# ─────────────────────────────────────────────────────────────
# 动态计算项目根目录
# 假设此文件在 <PROJECT_ROOT>/model/ 目录下
# 所以项目根目录 = 当前文件所在目录的上一级
# ─────────────────────────────────────────────────────────────
PROJECT_ROOT = os.path.dirname(os.path.dirname(os.path.abspath(__file__)))
COCO_DATA_DIR = os.path.join(PROJECT_ROOT, "data", "coco_data")
VG_DATA_DIR = os.path.join(PROJECT_ROOT, "data", "visual_genome_data", "vg_image")
...
class VG_Relation(Dataset):
    def __init__(self, transform=None):
        ...
        self.root_dir = VG_DATA_DIR  # ← 修改

        with open(os.path.join(DATA_DIR, "visual_genome_relation_aug.json"), "r") as f:  # ← 修改
    ...
    def __getitem__(self, index):
        ...
        reversed_head_inputs, reversed_relation_inputs, reversed_tail_inputs, reversed_attention_mask = self.get_triple_token_infor(
            reversed_triples)
    ...
    def get_triple_token_infor(self, triples):
        ...
        if len(head_word_list) > 0:
            attention_mask = torch.cat((
                torch.ones([1, len(head_word_list)], dtype=int),
                torch.zeros([1, self.padding_num - len(head_word_list)], dtype=int)
            ), dim=1)
        else:
        ...
        head_inputs = self.tokenizer.batch_encode_plus(
            head_word_list, max_length=self.length, add_special_tokens=True,
            padding="max_length", return_tensors="pt", truncation=True
        )
        relation_inputs = self.tokenizer.batch_encode_plus(
            relation_word_list, max_length=self.length, add_special_tokens=True,
            padding="max_length", return_tensors="pt", truncation=True
        )
        tail_inputs = self.tokenizer.batch_encode_plus(
            tail_word_list, max_length=self.length, add_special_tokens=True,
            padding="max_length", return_tensors="pt", truncation=True
        )
...
class VG_Attribution(Dataset):
    def __init__(self, data_path=None, transform=None):
        ...
        if 'adjchange' in data_path:
            self.root_dir = COCO_DATA_DIR
        else:
            self.root_dir = VG_DATA_DIR
    ...
    def __getitem__(self, index):
        ...
        # Get the bounding box that contains the relation
        # ← 修改条件判断逻辑
        if self.root_dir != COCO_DATA_DIR:
            image = image.crop((scene["bbox_x"], scene["bbox_y"], scene["bbox_x"] + scene["bbox_w"],
                                scene["bbox_y"] + scene["bbox_h"]))
