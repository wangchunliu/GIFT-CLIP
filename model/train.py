...
from eval import eval_coco, eval_coco_large, eval_coco_batch, test_vg_relation, test_vg_attribution, \
    test_vg_structural_retrieval_baseline
import wandb
import json
...
set_manualSeed(args)  # 固定随即种子，保证实验可以复现
...
for epoch in range(args.epoch):
    ...
    for i, batch in enumerate(tqdm(train_vg_dataloader, total=len(train_vg_dataloader))):
        ...
        if i % args.eval_interval == 0:
            ...
            # train.py  line 70~80
            t1, t5, t10, i1, i5, i10 = eval_coco_large(
                clip_model, myTransformer, test_dataloader, idx2id, args
            )
            metrics = {  # 一次性映射成 dict
                "TextR@1": t1, "TextR@5": t5, "TextR@10": t10,
                "ImageR@1": i1, "ImageR@5": i5, "ImageR@10": i10,
            }
            ...
            # eval task2
            acc_test_attribution = test_vg_attribution(clip_model, myTransformer, test_vg_dataloader, test_vg_dataset,
                                                       args)
            wandb.log({"acc_test_attribution": acc_test_attribution})

            acc_test_relation = test_vg_relation(clip_model, myTransformer, vg_relation_dataloader, vg_relation_dataset,
                                                 args)
            wandb.log({"acc_test_relation": acc_test_relation})

            attr_retrieval_metrics = test_vg_structural_retrieval_baseline(
                clip_model, myTransformer, args.topk_attribution_path, args, task_name="attribute"
            )
            if attr_retrieval_metrics:
                wandb.log(attr_retrieval_metrics)

            rel_retrieval_metrics = test_vg_structural_retrieval_baseline(
                clip_model, myTransformer, args.topk_relation_path, args, task_name="relation"
            )
            if rel_retrieval_metrics:
                wandb.log(rel_retrieval_metrics)
        ...
        knowledge_emb = myTransformer(batch["head_inputs"], batch["relation_inputs"], batch["tail_inputs"], 0,
                                      attention_mask)
        knowledge_emb = knowledge_emb / knowledge_emb.norm(dim=1, keepdim=True)
        logit_img2text, text_features, image_features = clip_model(img, text_true)  ##img/text
